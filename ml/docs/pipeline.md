...
GitHub에서는 자동으로 렌더링되며, VSCode에서는 Mermaid 플러그인으로 미리보기 가능.
...
새 스테이지로 진입할 때마다:
1. 이전 스테이지의 `class` 를 `done` 으로 변경
2. 현재 스테이지의 `class` 를 `current` 로 변경
3. `pipeline.mmd` 와 이 파일 둘 다 갱신
...
flowchart TB
    ...
    subgraph S1[Stage 1 추출 - 완료]
        direction TB
        S1_IN[입력: PDF 파일]
        S1_F1[pdf_extractor.py<br/>pymupdf 블록 추출]
        S1_F2[noise_filter.py<br/>좌표 기반 필터]
        S1_F3[scripts/01_extract.py<br/>CLI 실행]
        S1_OUT[출력<br/>extracted/pages.jsonl]
        S1_IN --> S1_F1 --> S1_F2 --> S1_F3 --> S1_OUT
    end

    S1 --> S2

    subgraph S2[Stage 2 섹션 감지 - 진행중]
        direction TB
    ...
    subgraph S5[Stage 5 LLM 정제]
        ...
        S5_F1[refinement/llm_client.py<br/>Gemma4 클라이언트]
...
- `ml/data_pipeline/pipeline.mmd` — 순수 Mermaid 코드 (이 파일에서 추출)
