...
### 3.4 문서 갱신 — Python heredoc 패턴

기존 문서(`.md`, 설정 파일 등)를 수정할 때는 **Python heredoc + assert 패턴** 사용. sed/vim 직접 편집보다 안전하고 검증 가능.
...
p = Path('ml/docs/EXAMPLE.md')
text = p.read_text(encoding='utf-8')

old = """기존 텍스트 블록 (정확히 일치해야 함)"""
new = """교체할 새 텍스트 블록"""

assert old in text, "기존 블록 못 찾음"
...
**원칙**:
1. **assert 필수** — 못 찾으면 파일 건드리지 않고 에러. 사일런트 실패 방지.
2. **여러 수정 한 번에** — 같은 파일에 여러 군데 수정하면 한 Python 블록에 묶기. 부분 실패 시 어디서 멈췄는지 명확.
3. **수정 후 grep 검증** — Python 블록 직후 `grep -n "키워드"`로 의도한 곳에 들어갔는지 확인.
4. **다중 줄은 triple-quote** — 한 줄 sed로 안 되는 경우 Python이 명확.
5. **숨은 공백 주의** — `old`에 들어가는 텍스트는 파일과 **공백·빈 줄까지 1:1 일치**해야 함.

**heredoc 중첩 금지**: 위 패턴 안에 또 다른 `python << PYEOF` 같은 heredoc 예시를 넣으면 안쪽 종료자가 바깥 heredoc을 끝내버림. 이런 메타 케이스는 `cat > /tmp/script.py << EOF` 로 임시 파일을 만든 뒤 `python /tmp/script.py` 로 실행할 것.

**공백 미스매치 fallback** — 줄 번호 동적 탐색:
...
lines[start_idx:end_idx] = new_block.splitlines(keepends=True)
p.write_text(''.join(lines), encoding='utf-8')
~~~

**금지**:
- ❌ `sed -i` 직접 사용 (특수문자 escape 지옥, 검증 어려움)
- ❌ 사용자에게 vim/nano로 수동 편집 요청 (실수 가능)
- ❌ 검증 없이 다음 작업 진행 (잔재 누적)
...
| "Stage C 결과 해석/분석하고 싶어" | `python ml/finetune/scripts/stage_c_analyze.py --help` 후 12 서브커맨드 활용 | 부분 |
